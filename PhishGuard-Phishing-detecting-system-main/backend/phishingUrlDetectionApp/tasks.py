...
class DatabaseUpdaterThread(threading.Thread):
    ...
    def _update_database(self):
        try:
            ...
            if not result:
                logger.error("Database update failed: No response returned")
                return
            ...
            if status == "success":
                ...
                logger.info(f"Database update skipped: {message}")
            else:
...
def start_database_updater():
    ...
    # 🔒 Only start if environment variable allows it
    if os.environ.get("PHISHGUARD_START_UPDATER", "1") != "1":
        logger.info("Database updater disabled via environment variable")
        return
...
# -----------------------------
# Optional External Feed Updates
# -----------------------------
        ...
        if response.status_code == 200:
            ...
            for phish_url in urls:
                try:
                    ...
                    if domain:
                        reputation_checker._update_cache(
                            phish_url, domain, True, 'openphish'
                        )
                        domains_added += 1
                except Exception:
                    continue

            logger.info(f"Added {domains_added} domains from OpenPhish")
            return True
        else:
            logger.warning(
                f"OpenPhish feed returned status code {response.status_code}"
            )
            return False
                ...
                for line in data_lines:
                    try:
                        ...
                        if len(fields) >= 3:
                            ...
                            if domain:
                                reputation_checker._update_cache(
                                    phish_url, domain, True, 'urlhaus'
                                )
                                domains_added += 1
                    except Exception:
                        continue

                logger.info(f"Added {domains_added} domains from URLhaus")
                return True

        else:
            logger.warning(
                f"URLhaus feed returned status code {response.status_code}"
            )
            return False

    except Exception as e:
        logger.error(f"Error fetching URLhaus feed: {str(e)}")
        return False


# Duplicate unsafe function removed intentionally
# def start_database_updater():
#     updater_thread = threading.Thread(target=phishing_database_updater, daemon=True)
#     updater_thread.start()
#     logging.info("Phishing database updater thread started")
